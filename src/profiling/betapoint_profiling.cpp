...
void MemProfiler::dumpStride(int bucketSize){
    ...
    for(int mem_type = 0;mem_type < 2;mem_type++){
        ...
        for(std::map<int64_t, int>::iterator iter = tempGlobalStride[mem_type].begin();iter != tempGlobalStride[mem_type].end();++iter){
            int64_t index = iter->first < 0 ? iter->first / bucketSize - 1 : iter->first / bucketSize;
            if (globalStride[mem_type].find(index) == globalStride[mem_type].end()) {
                globalStride[mem_type][index] = iter->second;
            } else {
                globalStride[mem_type][index] = globalStride[mem_type][index] + iter->second;
            }
        }
    ...
    for(int mem_type = 0;mem_type < 2;mem_type++){
        ...
        for(std::map<vaddr_t, std::vector<std::pair<int64_t, int> > >::iterator iter = tempLocalStride[mem_type].begin();iter != tempLocalStride[mem_type].end();++iter){
            ...
            for(std::vector<std::pair<int64_t, int> >::iterator vecIter = vec.begin();vecIter != vec.end();++vecIter){
                ...
                if (localStride[mem_type].find(index) == localStride[mem_type].end()) {
                    localStride[mem_type][index] = vecIter->second;
                } else {
                    localStride[mem_type][index] = localStride[mem_type][index] + vecIter->second;
                }
            }
        }
        for(std::map<int64_t, int>::iterator iter = localStride[mem_type].begin();iter != localStride[mem_type].end();++iter){
            if (iter->first >= 0) {
                ss << std::hex << iter->first * bucketSize << "~" << (iter->first + 1) * bucketSize - 1 << " " << std::oct << iter->second << std::endl;
            } else {
                ss << std::hex << "-" << -iter->first * bucketSize << "~-" << (-iter->first - 1) * bucketSize + 1 << " " << std::oct << iter->second << std::endl;
